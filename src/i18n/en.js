export const en = {
  common: {
    next: 'Next →',
    back: '← Back',
    backToMenu: '← Back to menu',
  },

  phases: { night: 'Night', day: 'Day', vote: 'Vote' },

  sides: {
    dark: 'Dark side',
    light: 'Light side',
    solo: 'Lone wolf',
  },

  roles: {
    mafia: {
      name: 'Mafia',
      desc: 'Each night you and your accomplices pick a victim. By day, blend in with the townsfolk and throw everyone off.',
    },
    civilian: {
      name: 'Civilian',
      nameLabel: 'Civilians',
      desc: 'You have no special powers. Your weapons are logic, intuition and persuasion. Find the mafia before it is too late.',
    },
    sheriff: {
      name: 'Sheriff',
      desc: 'Each night you check one player — the host shows whether they are mafia. Beware: once the mafia knows you, you become their top target.',
    },
    doctor: {
      name: 'Doctor',
      desc: 'Each night you heal one player — they survive any night attack (mafia, the Maniac, or the Veteran’s strike). You cannot heal the same player two nights in a row. You can heal yourself only once per game.',
    },
    don: {
      name: 'Mafia Don',
      nameShort: 'Don',
      desc: 'You lead the mafia. Alongside the kills, each night you can check whether a player is the Sheriff.',
    },
    maniac: {
      name: 'Maniac',
      desc: 'You play for yourself. Every night, including the first, you kill one player. You win when the mafia is gone and at most one civilian remains.',
    },
    whore: {
      name: 'Whore',
      desc: 'Each night you pick a player — they "sleep over" and cannot use their night ability; they have an alibi for that day and cannot be voted out. No visits to yourself; no same target two nights in a row. At the mafia you do not die, but you block them only if that was their last living member.',
      descDies: 'Each night you pick a player — they "sleep over" and cannot use their night ability; they have an alibi for that day and cannot be voted out. No visits to yourself; no same target two nights in a row. Careful: visit the mafia and you die — but the mafia kills no one that night.',
    },
    veteran: {
      name: 'Veteran',
      desc: 'Once per game, at night, you save any player from death; once per game — you kill anyone (not yourself). Strike the Maniac before his turn — he does not kill that night. Reads as civilian to the Sheriff.',
    },
  },

  validation: {
    playerMissing: 'Player not found',
    doctorSameTarget: 'The Doctor healed this player last night',
    doctorSelfLimit: 'You can heal yourself only once per game',
    whoreSelf: 'The Whore cannot visit herself',
    whoreSameTarget: 'The Whore already visited this player last night',
    veteranHealUsed: 'The save has already been used this game',
    veteranKillUsed: 'The strike has already been used this game',
    veteranSelfKill: 'The Veteran cannot strike himself',
  },

  steps: {
    cityFallsAsleep: {
      title: 'The town falls asleep',
      say: 'The town falls asleep. Everyone closes their eyes. Silence, please.',
      hint: 'Wait for full silence. A steady tap on the table masks the rustle from active roles.',
    },
    mafiaMeet: {
      title: 'Mafia gets acquainted',
      say: 'Mafia, wake up. Meet each other silently — with eyes and nods. Tonight the mafia does NOT kill.',
      hint: 'Give them 10–15 seconds. Then say: "Mafia, close your eyes."',
    },
    whore: {
      title: 'Whore',
      say: 'Whore, open your eyes. Who are you visiting tonight?',
      hint: 'After the pick — "Whore, close your eyes."',
      rules: 'She cannot visit herself; no same target two nights in a row.',
      tipsDies: 'Visits a mafioso — the Whore dies, and the mafia kills no one that night.',
      tipsAlive: 'Her target’s night action will not fire. Visits a mafioso — she has an alibi; the mafia is blocked only if that was their last living member.',
      label: 'Whom the Whore visits',
    },
    mafiaKill: {
      title: 'Mafia wakes up',
      say: 'Mafia, wake up. Choose your victim with gestures.',
      closingDon: 'Next up is the Don — the mafia stays awake.',
      closingAlone: 'After the pick — "Mafia, close your eyes."',
      hintBlocked: 'The Whore blocked the mafia. Take their gesture, hold the usual pause — no reaction, no kill. {closing}',
      hint: 'Wait for a unanimous decision. If they do not agree — "Mafia failed to agree." {closing}',
      blockedLabel: 'Mafia does not kill tonight',
      blockedConfirm: 'Mafia kills no one',
      label: 'Mafia victim',
      skipLabel: 'Mafia failed to agree',
      timerLabel: 'Mafia has to pick',
    },
    don: {
      title: 'The Don looks for the Sheriff',
      opening: 'Don, wake up. ',
      closingFirst: 'Then say: "Don, close your eyes."',
      closingLater: 'Then say: "Mafia, close your eyes" — all mafiosi and the Don close their eyes together.',
      say: '{opening}Point to whom you are checking.',
      hintBlocked: 'The Whore blocked the Don. Take the gesture — do NOT return an answer, keep the usual rhythm. {closing}',
      hint: 'Look the role up in the roster below — is it the Sheriff? Signal the Don with a gesture: nod — yes, shake — no. {closing}',
    },
    doctor: {
      title: 'Doctor',
      say: 'Doctor, wake up. Whom are you healing tonight?',
      hintBlocked: 'The Whore blocked the Doctor. Take the gesture — no reaction, no heal. Then — "Doctor, close your eyes."',
      hint: 'After the pick — "Doctor, close your eyes."',
      rules: 'No healing the same player two nights in a row. Self-heal — once per game.',
      tips: 'The Doctor heals against the mafia, the Maniac, and the Veteran’s strike.',
      blockedLabel: 'Doctor does not heal tonight',
      blockedConfirm: 'Doctor heals no one',
      label: 'Whom the Doctor heals',
      skipLabel: 'Heal no one',
    },
    sheriff: {
      title: 'Sheriff',
      say: 'Sheriff, wake up. Whom are you checking?',
      hintBlocked: 'The Whore blocked the Sheriff. Take the gesture — do NOT return an answer, keep the usual rhythm. Then — "Sheriff, close your eyes."',
      hint: 'Look the role up in the roster below — mafia or not. Signal the Sheriff with a gesture: thumb up — NOT mafia, thumb down — mafia. Then — "Sheriff, close your eyes."',
    },
    maniac: {
      title: 'Maniac',
      say: 'Maniac, open your eyes. Whom are you killing?',
      hintBlocked: 'The Whore blocked the Maniac. Take the gesture — no reaction, no kill. Then — "Maniac, close your eyes."',
      hint: 'After the pick — "Maniac, close your eyes."',
      tips: 'The Doctor can save a target from the Maniac.',
      tipsFirst: 'The Maniac acts on the first night too. The Doctor can save a target from him.',
      blockedLabel: 'Maniac does not kill tonight',
      blockedConfirm: 'Maniac kills no one',
      label: 'Maniac victim',
      skipLabel: 'Maniac skips',
    },
    veteran: {
      title: 'Veteran',
      say: 'Veteran, open your eyes. What do you do tonight?',
      hintBlocked: 'The Whore blocked the Veteran — the attempt is burned. Take the gesture — no reaction. Then — "Veteran, close your eyes."',
      hint: 'After the pick — "Veteran, close your eyes."',
      rules: 'Across the whole game: one save and one strike, on different nights.',
      tips: 'The Doctor heals against the Veteran too. Strike the Maniac before his turn — the Maniac does not act tonight.',
      modeLabel: 'What the Veteran does',
      modeSave: '⛨ Save',
      modeKill: '✖ Strike',
      modeSkip: 'Skip the night',
      usedSave: '(save already used)',
      usedKill: '(strike already used)',
      labelSave: 'Whom he shields',
      labelKill: 'Whom he strikes',
      blockedLabel: 'Veteran takes no action tonight',
      blockedConfirm: 'Veteran skips his turn',
    },
    dawn: {
      title: 'Dawn',
      say: 'The town wakes up. Open your eyes.',
      hint: 'Read the text above aloud to the town. The private summary below is for your eyes only: "Night outcome", "Night actions", "Effects on today". Tap "Next" to apply the result and move to day.',
      peacefulFirst: [
        'Good morning, town. Everyone is intact — the family must still be sniffing around.',
        'Good morning, town. The first night passed without blood. Time to get acquainted.',
        'Good morning, town. A miracle: everyone is alive. Make it count.',
      ],
      peacefulLater: [
        'Good morning, town. Day {day}. Against the odds, no one died tonight.',
        'Good morning, town. Day {day}. The town held — not a single loss.',
        'Good morning, town. Day {day}. A quiet night; everyone is alive.',
      ],
      deathFirstOne: [
        'Good morning, town. Grim news: on the very first night we lost {names}.',
        'Good morning, town. The family showed its teeth right away — {names} is gone.',
        'Good morning, town. First casualty: {names}. The town is no longer safe.',
      ],
      deathFirstMany: [
        'Good morning, town. Grim news: on the very first night we lost {names}.',
        'Good morning, town. The family showed its teeth right away: {names}.',
        'Good morning, town. First casualties — {names}. The town is no longer safe.',
      ],
      deathLaterOne: [
        'Good morning, town. Day {day}. Tonight we lost {names}.',
        'Good morning, town. Day {day}. A new loss: {names}.',
        'Good morning, town. Day {day}. Another victim — {names}.',
      ],
      deathLaterMany: [
        'Good morning, town. Day {day}. Tonight we lost {names}.',
        'Good morning, town. Day {day}. New losses: {names}.',
        'Good morning, town. Day {day}. The night took a grim toll: {names}.',
      ],
    },
    victimsTitle: 'Last words',
    victimsLastWordOne: [
      'Last words — <strong>{names}</strong>.',
      'The floor goes to the fallen — <strong>{names}</strong>.',
      'A moment of memory. <strong>{names}</strong> speaks.',
    ],
    victimsLastWordMany: [
      'Last words in turn — <strong>{names}</strong>.',
      'The floor goes to the fallen, in turn — <strong>{names}</strong>.',
      'A moment of memory. In turn, we hear <strong>{names}</strong>.',
    ],
    victimsHint: 'Give 30 seconds for last words — the deceased may reveal their role.',
    victimsTimerLabel: "Deceased's last words",
    discussionTitle: 'Discussion',
    discussionSay: 'Time to discuss. Each player gets a minute to speak, going around the table.',
    discussionHintFirst: 'The first day is usually short — players know almost nothing. Start with anyone.',
    discussionHint: 'Start with the player to the left of the first deceased. Keep strict time. No interruptions.',
    discussionTimerLabel: 'A minute per player',
    nominationTitle: 'Nominations and defence',
    nominationSay: 'Whom do you suspect? Nominate candidates. Each gets 30 seconds to defend themselves.',
    nominationHint: 'One player nominates one other. Self-nomination is not allowed. If no one is nominated — no vote, straight to night. Run the timer for each candidate separately.',
    nominationTimerLabel: '30 seconds per candidate',
    voteTitle: 'Vote',
    voteSay: 'Voting. Raise your hand for the player you suspect.',
    voteHint: 'Name candidates one by one. Each votes once, not for themselves. Count aloud. On a tie, revote verbally between the leaders; if still tied — no one leaves or all leaders leave (agree before the game). Pick the executed player below or "No one leaves".',
    voteLabel: 'Whom the town executed',
    voteSkipLabel: 'No one leaves',
    voteLastWordTitle: 'Last words',
    voteLastWordSay: '{name}, you have 30 seconds for last words.',
    voteLastWordHint: 'The executed player has 30 seconds — they may reveal their role. Next step — night.',
    voteLastWordTimerLabel: 'Last words',
  },

  home: {
    ornament: 'C O S A · N O S T R A',
    year: 'Est. 1986',
    title: 'Mafia',
    titleEm: 'in famiglia',
    subtitle: 'The town falls asleep —<br>the mafia wakes',
    sectionPlayers: 'Players at the table',
    playersUnit: 'people',
    sectionRoles: 'Role spread',
    sectionExtra: 'Optional roles',
    startBtn: 'Deal roles',
    rulesBtn: 'Game rules',
    resumeKicker: 'Saved game',
    resumeContinue: 'Continue →',
    resumeDelete: 'Delete',
    resumeConfirmDelete: 'Delete the saved game?',
    roleDesc: {
      don: 'Checks the Sheriff at night. Minimum 6 players.',
      doctor: 'Heals one player per night.',
      maniac: 'Lone wolf. Kills for himself. Minimum 8 players.',
      whore: 'Blocks night abilities. Minimum 8 players.',
      veteran: 'Saves once, strikes once. Minimum 6 players.',
    },
    warnMinPlayers: 'Requires at least {n} players',
    warnSqueezed: '⚠ Will not fit — no slot in the current spread',
    suboption: {
      whoreAtMafia: 'Whore visiting the mafia',
      whoreStaysAlive: 'Stays alive',
      whoreDies: 'Dies',
    },
  },

  install: {
    title: 'Install the app',
    promptBody: 'Add Mafia to your home screen — works offline and launches as a standalone app.',
    iosHint: 'Tap the Share icon, then "Add to Home Screen" to install the app.',
    accept: 'Install',
    dismissAria: 'Dismiss',
    footerLink: 'Install',
  },

  names: {
    ornament: 'D R A M A T I S · P E R S O N A E',
    title: 'Names',
    titleEm: 'at the table',
    subtitle: 'Enter names or keep the defaults',
    placeholder: 'Player {n}',
    defaultName: 'Player {n}',
    confirmBtn: 'Deal cards',
    clearRoster: 'Clear list',
  },

  deal: {
    handoffKicker: 'All roles dealt',
    handoffTitle: 'Hand the phone<br>to the host',
    handoffHint: 'only the host should see the next screen — it shows every role',
    handoffBtn: 'I am the host — start',
    playerKicker: 'Player · {num} / {total}',
    passHint: 'hand the phone to this player',
    revealBtn: 'Reveal<br>role',
    backKicker: 'Your role',
    backHint: 'tap to reveal',
    instruction: 'Make sure no one<br>is looking over&nbsp;your shoulder',
    teamLabel: 'Your accomplices',
    nextBtn: 'Got it, passing on',
    lastBtn: 'Got it, passing to the host',
  },

  host: {
    phaseDay: 'Day {day} · {phase}',
    hostSays: 'Host says',
    stepNum: 'Step {current} / {total}',
    playersHeader: 'Players · alive {alive}/{total}',
    endGame: 'End game',
    endConfirm: 'End the game and return to the menu?',
    nextPhaseNight: 'To day →',
    nextPhaseDay: 'To vote →',
    nextPhaseVote: 'To night {day} →',
  },

  actions: {
    blockedNote: '❀ This action is blocked by the Whore tonight',
    blockedConfirmed: '✓ Confirmed',
    blockedConfirm: 'Confirm',
    skipDefault: 'Skip',
    mark: {
      mafia: 'victim',
      doctor: 'save',
      whore: 'visit',
      veteranSave: 'shield',
      veteranKill: 'strike',
      maniac: 'victim',
      vote: 'execute',
    },
    checkResultLabel: 'Check result',
    alibiKicker: 'Whore alibi',
    alibiBody: '<strong>{name}</strong> spent the night with the Whore — cannot be voted out today.',
    resolveTitle: 'Private summary',
    resolveSubtitle: 'Host only — do not read aloud.',

    // Section headers
    resolveSectionOutcome: 'Night outcome',
    resolveSectionActions: 'Night actions',
    resolveSectionEffects: 'Effects on today',

    // Block 1: outcome
    resolvePeaceful: '☾ No one died tonight',
    resolveKilledLine: '☠ <strong>{name}</strong> <em>({role})</em> — {cause}',
    resolveCauseMafia: 'killed by the mafia',
    resolveCauseManiac: 'killed by the Maniac',
    resolveCauseVeteran: 'killed by the Veteran',
    resolveCauseMafiaManiac: 'killed by the mafia and the Maniac',
    resolveCauseMafiaVeteran: 'killed by the mafia and the Veteran',
    resolveCauseManiacVeteran: 'killed by the Maniac and the Veteran',
    resolveCauseAll: 'killed by the mafia, Maniac, and Veteran',
    resolveCauseWhoreAtMafia: 'died at the mafia',

    // Block 2: actions per role
    resolveMafiaKill: '⚜ Mafia → <strong>{name}</strong> <em>({role})</em> — killed',
    resolveMafiaSavedDoctor: '⚜ Mafia → <strong>{name}</strong> <em>({role})</em> — Doctor saved',
    resolveMafiaSavedVeteran: '⚜ Mafia → <strong>{name}</strong> <em>({role})</em> — Veteran shielded',
    resolveMafiaBlocked: '⚜ Mafia — Whore blocked, no kill',
    resolveMafiaSkipped: '⚜ Mafia failed to agree — no kill',

    resolveManiacKill: '☠ Maniac → <strong>{name}</strong> <em>({role})</em> — killed',
    resolveManiacSavedDoctor: '☠ Maniac → <strong>{name}</strong> <em>({role})</em> — Doctor saved',
    resolveManiacSavedVeteran: '☠ Maniac → <strong>{name}</strong> <em>({role})</em> — Veteran shielded',
    resolveManiacBlocked: '☠ Maniac — Whore blocked, no kill',
    resolveManiacSkipped: '☠ Maniac picked no one',
    resolveManiacPreempted: '⛨ Veteran cut off the Maniac — his kill is cancelled',

    resolveDoctorSaved: '✚ Doctor → <strong>{name}</strong> <em>({role})</em> — saved {from}',
    resolveDoctorNoAttack: '✚ Doctor → <strong>{name}</strong> <em>({role})</em> — no attack came',
    resolveDoctorBlocked: '✚ Doctor — Whore blocked, no heal',

    resolveFromMafia: 'from the mafia',
    resolveFromManiac: 'from the Maniac',
    resolveFromVeteran: 'from the Veteran',
    resolveFromMafiaManiac: 'from the mafia and the Maniac',
    resolveFromMafiaVeteran: 'from the mafia and the Veteran',
    resolveFromManiacVeteran: 'from the Maniac and the Veteran',
    resolveFromAll: 'from the mafia, Maniac, and Veteran',

    resolveWhoreToMafiaDied: '❀ Whore at the mafia — died',
    resolveWhoreToMafiaSaved: '❀ Whore at the mafia — would have died, but the Doctor saved her',
    resolveWhoreToMafiaAliveBlocked: '❀ Whore at the mafia — alive; the mafia does not kill tonight',
    resolveWhoreToMafiaAliveNotBlocked: '❀ Whore at the mafia — alive; the mafia kills freely',

    resolveVeteranSaveHelped: '⛨ Veteran shielded <strong>{name}</strong> <em>({role})</em> — saved {from}',
    resolveVeteranSaveWasted: '⛨ Veteran shielded <strong>{name}</strong> <em>({role})</em> — no attack came, the save is burned',
    resolveVeteranKill: '⛨ Veteran struck down <strong>{name}</strong> <em>({role})</em>',
    resolveVeteranBlocked: '⛨ Veteran — Whore blocked, the attempt is burned',

    resolveSheriffBlocked: '✦ Sheriff — Whore blocked the check',
    resolveDonBlocked: '♛ Don — Whore blocked the check',

    // Block 3: day effects
    resolveAlibi: '❀ Alibi: <strong>{name}</strong> <em>({role})</em> — cannot be voted out today (Whore’s visit)',
  },

  timer: {
    label: 'Timer',
    pause: '⏸ Pause',
    start: '▶ Start',
    minus10: '−10s',
    plus10: '+10s',
    resetTo: 'Reset to {time}',
  },

  gameover: {
    finalKicker: 'Finale',
    allParticipants: 'All participants',
    newGame: 'New game',
    history: {
      title: 'Night log',
      nightHeader: 'Night {day}',
      peaceful: '☾ A quiet night',
      killedOne: '✖ Dead: {names}',
      killedMany: '✖ Dead: {names}',
      savedByDoctor: '✚ Doctor saved: {name}',
      mafiaPick: '⚜ Mafia picked: {name}',
      maniacPick: '☠ Maniac picked: {name}',
      whoreVisit: '❀ Whore → {name}',
      whoreDied: '❀ The Whore died at the mafia',
      whoreSaved: '❀ The Doctor saved the Whore',
      veteranSave: '⛨ Veteran shielded: {name}',
      veteranKill: '⛨ Veteran struck down: {name}',
      veteranPreempt: '⛨ Veteran cut off the Maniac',
      veteranBlocked: '⛨ The Whore blocked the Veteran',
      blocked: '❀ Whore blocked: {list}',
    },
    verdict: {
      cityText: 'Town<br>wins',
      citySub: 'The mafia has been cleansed from the town. Justice has prevailed.',
      mafiaText: 'Mafia<br>wins',
      mafiaSub: 'The town is in the family’s hands. New laws rule here now.',
      maniacText: 'Maniac<br>wins',
      maniacSub: 'The last one standing. Everyone else — in the graveyard.',
      drawText: 'Draw',
      drawSub: 'No one survived to tell the tale. The town stands empty.',
    },
  },

  rules: {
    ornament: 'R E G U L A · L U D I',
    title: 'Rules',
    titleEm: 'of the game',
    sections: {
      essence: {
        h: 'The gist',
        body: '<p>Some players are mafia — they know each other and "kill" at night. The rest are civilians — they know no one and try to identify the mafia by daytime vote. Whoever moves faster wins: the mafia by tipping the numbers, the town by voting the family out to the last member.</p>',
      },
      roles: {
        h: 'Roles in play',
        body: '<p>Every role belongs to a side — dark, light, or lone wolf. Some are always in the game; others are toggled on from the home screen.</p><ul><li><strong>⚜ Mafia</strong> <em>(dark side)</em> — knows the other mafiosi by sight. From the second night, the family silently picks a victim and kills. By day, the mafia blends in with the civilians, votes with them, and steers suspicion elsewhere.</li><li><strong>♛ Mafia Don</strong> <em>(dark side, 6+ players, requires at least 2 mafia slots)</em> — head of the family. Joins the nightly kills and, in addition, each night checks one player to see whether they are the Sheriff. To the Sheriff, the Don looks like any other mafioso.</li><li><strong>♟ Civilian</strong> <em>(light side)</em> — no special powers. Logic, observation, and persuasion are the whole kit. The job is to vote out every member of the mafia alongside the rest of the town.</li><li><strong>✦ Sheriff</strong> <em>(light side, always in play)</em> — each night points at one player; the host checks the role in the roster and signals the Sheriff by hand: thumb up — not mafia, thumb down — mafia. The Don and ordinary mafiosi read as mafia; the Veteran and the Maniac read as NOT mafia.</li><li><strong>✚ Doctor</strong> <em>(light side, optional)</em> — each night heals one player; if that player is attacked by the mafia, the Maniac, or the Veteran, they survive. Cannot heal the same player two nights in a row. Self-heal is allowed only once per game.</li><li><strong>❀ Whore</strong> <em>(light side, 8+ players)</em> — each night visits one player, and that player loses their night ability (mafia cannot kill, Sheriff cannot check, Maniac cannot strike, etc.). The visited player gets an alibi for that day — they cannot be voted out. She cannot visit herself, and cannot visit the same player two nights in a row. What happens when she visits the mafia is set by the "Whore at the mafia" option (see edge cases).</li><li><strong>☠ Maniac</strong> <em>(lone wolf, 8+ players)</em> — plays for himself. Every night, including the first, kills one player. He is not part of the mafia and does not know them. Wins if the mafia is gone and at most one civilian remains.</li><li><strong>⛨ Veteran</strong> <em>(light side, 6+ players)</em> — a retired agent who cannot sit still. Across the whole game, once at night he can shield any player from death (himself included) and once he can kill anyone (except himself). The two attempts must fall on different nights — there is no third. The Doctor heals against the Veteran’s strike as well. If the Veteran kills the Maniac before the Maniac’s turn, the Maniac does not act that night. The Sheriff reads the Veteran as a civilian.</li></ul>',
      },
      cycle: {
        h: 'Game cycle',
        body: '<ul><li><strong>Night:</strong> active roles act in turn — Whore, Mafia, Don, Doctor, Sheriff, Maniac, Veteran.</li><li><strong>Day:</strong> announcement of victims, round-robin discussion, nominations and defences.</li><li><strong>Vote:</strong> the town votes on whom to execute; the executed player gets last words.</li></ul>',
      },
      winConditions: {
        h: 'Win conditions',
        body: '<ul><li><strong>Civilians</strong> (Sheriff, Doctor, Whore, Veteran included): win once no mafia member and no Maniac remain in the town.</li><li><strong>Mafia</strong> (mafiosi and the Don): win once the number of living mafiosi equals or exceeds the number of living civilians.</li><li><strong>Maniac:</strong> wins if he is still alive, the mafia is gone, and at most one civilian remains.</li></ul>',
      },
      nightEdge: {
        h: 'Night edge cases',
        body: '<ul><li><strong>Mafia fails to agree:</strong> no kill tonight — the family simply skips.</li><li><strong>Doctor heals the target:</strong> the Doctor saves from the mafia, the Maniac, and the Veteran’s strike alike. If two attackers pick the same target, one heal still saves them.</li><li><strong>Whore’s alibi:</strong> whoever the Whore visits that night cannot be voted out the next day — they have an alibi at the table.</li><li><strong>Whore at the mafia — "dies":</strong> the whole mafia kills no one that night, and the Whore dies (the Doctor can still save her). The Don’s personal check is blocked too.</li><li><strong>Whore at the mafia — "stays alive":</strong> the Whore does not die. The whole mafia is blocked only if she visited their last living member; otherwise the rest kill as usual. The Don’s check is blocked only when she visits the Don himself.</li><li><strong>Whore blocks the Doctor/Sheriff/Maniac/Veteran:</strong> their night action does not trigger. The Veteran’s attempt (save or strike) still burns — it still counts as one of his two.</li><li><strong>Maniac and mafia on the same target:</strong> if the Doctor heals that player — they live; otherwise they die (counts as a single kill).</li><li><strong>Veteran strikes the Maniac:</strong> the Maniac does not act that night. If the Doctor heals the Maniac, the Maniac lives but his night strike is still cancelled.</li><li><strong>First night:</strong> the mafia meets and does not kill; a Whore visiting the mafia does not die. The Maniac, Sheriff, Doctor, and Veteran act as normal.</li><li><strong>Sheriff checks the Don:</strong> reads as mafia.</li><li><strong>Sheriff checks the Maniac:</strong> reads as NOT mafia.</li><li><strong>Sheriff checks the Veteran:</strong> reads as NOT mafia.</li></ul>',
      },
      voteEdge: {
        h: 'Vote edge cases',
        body: '<ul><li><strong>No one nominated:</strong> skip the vote, straight to night.</li><li><strong>Single candidate:</strong> either vote for/against, or execute without a vote (agree beforehand).</li><li><strong>Tied votes:</strong> revote verbally between the leaders. If still tied — no one leaves OR all leaders leave (decide before the game).</li><li><strong>You cannot vote:</strong> for yourself, or if you are dead.</li><li><strong>Last words:</strong> the executed player gets 30 seconds — may reveal their role.</li></ul>',
      },
      general: {
        h: 'General rules',
        body: '<ul><li>Dead players do not tip off the living — not by word, gesture, or expression.</li><li>Do not show your card or role to others during the game.</li><li>The mafia gets acquainted on the first night (no kill); the Maniac and the Veteran already act on the first night.</li><li>Do not hint at your check, heal, or strike with gestures, tone, or expression.</li><li>The host is the final authority — their decisions are final.</li></ul>',
      },
      hostTips: {
        h: 'Host tips',
        body: '<ul><li>Keep the same rhythm and intonation across every night step — any deviation gives away a Whore block or the Sheriff’s verdict.</li><li>Signalling the Sheriff or the Don, do not slow down or speed up: the response for "mafia" and "not mafia" must look identical.</li><li>Dramatise the dawn announcements — you are the voice of the town, it sets the mood and keeps the table engaged.</li><li>State the "Whore at the mafia" setting out loud before the deal — players must understand the risk upfront, otherwise complaints after the night.</li><li>A player accidentally leaked their role — judge by the situation: usually play continues; in bad cases, redeal.</li></ul>',
      },
    },
  },

  resume: {
    justNow: 'just now',
    minAgo: '{n} min ago',
    hourMinAgo: '{h} h {m} min ago',
    description: '{phase} · Day {day} · Alive {alive}/{total}',
    descriptionDeal: 'Dealing cards · {current}/{total}',
  },

  theme: {
    toggleAria: 'Toggle theme',
  },

  locale: {
    label: 'Language',
    toggleAria: 'Change language',
  },
};
