...
- Lifehacker: «При проверке комиссаром показывается как мирный житель» — i.e.
  `sheriffSeesManiac = 'never'`.
- mafiasyndicate: ambiguous — default varies by club.
- Our app's default is `'afterMafia'` (Maniac shows as mafia only after the
  mafia team is dead), which matches several club variants but is *not* the
  Lifehacker default.
